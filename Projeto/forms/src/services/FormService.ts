...
export const getFormDetails = async (id: string): Promise<AxiosResponse> => {
  const response = await axiosInstance.get(`/formDetails/${id}/`);
  return response;
};
...
export const answerForm = async (AnswersData: Answers) => {
  ...
  AnswersData.answers.forEach(async (answer) => {
    await axiosInstance.post("/userAnswers/", {
...
export const getAnswersByFormId = async (
  ...
  const response = await axiosInstance.get(`/userAnswersByForm/${id}/`);
  return response;
};
