        ...
        try:
            ...
            if aluno.voteCounter < aluno.maxVotes:
                ...
                return render(
                    request,
                    "votacao/detalhe.html",
                    {"questao": questao, "errorMessage": errorMessage},
                )
    ...
    if request.user.is_superuser:
        if request.method == "POST":
            if request.POST["question"]:
                q = Questao(
                    questao_texto=request.POST["question"], pub_data=timezone.now()
                )
                q.save()
...
def createOption(request, questao_id):
    ...
    return render(
        request,
        "votacao/option.html",
        {"questao_id": questao_id, "errorMessage": errorMessage, "opcoes": opcoes},
    )
...
class LoginView(APIView): 
    def post(self, request): 
        ...
        if user: 
            ...
            return JsonResponse({'token': token.key, 'message': 'User logado!'}) 
        else: 
    ...
    if request.method == "POST":
        if request.POST["register"]:
            ...
            if (
                ...
                errorMessage = (
                    "Não inseriu todos os campos necessários, tente novamente!"
                )
...
def personalInfo(request):
    ...
    if user.is_authenticated:
        ...
        return render(
            request,
            "votacao/personalInfo.html",
            {"aluno": aluno, "remaining": remainingVotes},
        )
    else:
        errorMessage = (
            "Tem de ter efetuado o LogIn para poder aceder às suas informações"
        )
        return render(
            request, "votacao/personalInfo.html", {"errorMessage": errorMessage}
        )
